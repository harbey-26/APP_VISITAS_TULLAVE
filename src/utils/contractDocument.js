...
// formulario interpolados) a partir de las plantillas legales de TuLlave.
// Los bloques son agnósticos del medio: la vista previa HTML y el PDF
// (jspdf) los renderizan cada uno a su manera.
//
// Bloques: { kind: 'title'|'subtitle'|'table'|'paragraph'|'signature', ... }
//  - table: { rows: [[etiqueta, valor], ...] }  (cuadro resumen)
//  - signature: { role, lines: [string] }
//
// El texto legal se transcribió de los contratos vigentes de la inmobiliaria
// (julio 2026). Si el abogado cambia una cláusula, se edita aquí.
...
import { fechaCorta, fechaEnLetras } from './fechaLetras.js';
...
function buildAdministracion(d) {
    ...
    blocks.push({ kind: 'title', text: 'CONTRATO DE ADMINISTRACIÓN DE INMUEBLE' });

    const linderos = `Obran en Escritura Pública No. ${v(d.escrituraNumero)} de fecha ${fecha(d.escrituraFecha)} de la ${v(d.escrituraNotaria)}`;
    const duracion = `${mesesEnLetras(d.duracionMeses)} — Inicio: ${fecha(d.fechaInicio)} · Terminación: ${fecha(d.fechaTerminacion)}`;

    blocks.push({
        kind: 'table',
        rows: [
            ['Ciudad y fecha', `${v(d.ciudadFirma)}, ${fecha(d.fechaFirma)}`],
            ['Administrador', `${EMPRESA.razonSocial} — NIT ${EMPRESA.nit}`],
            ...
            ['Dirección del inmueble', v(d.direccionInmueble)],
            ['Conjunto / Edificio', v(d.conjunto)],
            ...
            ['Fianza integral o póliza de arrendamiento', `${v(d.fianzaPct)}% más IVA 19%`],
            ['Garajes', d.garajes ? `SÍ — No. ${v(d.numeroGarajes)}` : 'NO'],
            ['Depósito', d.deposito ? `SÍ — No. ${v(d.numeroDeposito)}` : 'NO'],
            ['Gravámenes / Limitaciones', d.gravamenes ? `SÍ — ${v(d.tipoGravamen)}` : 'NO'],
            ['Régimen de propiedad horizontal', siNo(d.regimenPH)],
    ...
    blocks.push({
        ...
        lines: [
            ...
            `CÉDULA: ${EMPRESA.cedulaRepresentante} — Representante legal`,
...
function buildArrendamiento(d) {
    ...
    blocks.push({
        ...
        lines: [
            ...
            `Tel. oficina: ${EMPRESA.telefono} — Celular: ${EMPRESA.celular}`,
    ...
    blocks.push({
        ...
        lines: [
            ...
            `C.C. No. ${v(d.arrendatarioCedula)} de ${v(d.arrendatarioLugarExpedicion)}`,
    ...
    deudores.forEach((x, i) => {
        blocks.push({
            ...
            lines: [
                `NOMBRE: ${v(x.nombre)}`,
                `C.C. No. ${v(x.cedula)} de ${v(x.lugarExpedicion)}`,
...
// Devuelve { title, blocks } o null si el tipo no existe.
export function buildContractDocument(type, data) {
    const template = getTemplate(type);
    if (!template) return null;
    const d = data || {};
    const blocks = type === 'ADMINISTRACION' ? buildAdministracion(d) : buildArrendamiento(d);
    return { title: template.label, blocks };
}
