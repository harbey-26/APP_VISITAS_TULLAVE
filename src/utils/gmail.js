...
import { buildMimeMessage, buildTextMimeMessage, buildMimeMessageAttachments } from './gmailMime.js';
import { explainGmailSendError } from './gmailErrors.js';

const GMAIL_SEND_URL = 'https://gmail.googleapis.com/gmail/v1/users/me/messages/send';

// P1: correo con N adjuntos de cualquier tipo (respuesta del derecho de
// petición). attachments = [{ base64, filename, mimeType }].
export async function sendEmailWithAttachments({ to, subject, text, attachments }) {
    ...
    if (!session.scope.includes('gmail.send')) {
        throw new Error('La cuenta de Google conectada no tiene permiso de envío de correo. Un administrador debe desconectar y volver a conectar Google en Ajustes.');
    }
    const mime = buildMimeMessageAttachments({ from: session.accountEmail, to, subject, text, attachments });
...
// P1: correo de solo texto (código OTP del Portal de Clientes). Mismos
// requisitos que sendEmailWithPdf: cuenta Google conectada con gmail.send.
export async function sendTextEmail({ to, subject, text }) {
    ...
    const mime = buildTextMimeMessage({ from: session.accountEmail, to, subject, text });
    const raw = Buffer.from(mime, 'utf8').toString('base64url');
    const r = await fetch(GMAIL_SEND_URL, {
        method: 'POST',
        headers: { Authorization: `Bearer ${session.token}`, 'Content-Type': 'application/json' },
