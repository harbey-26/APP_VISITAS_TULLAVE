...
| `ASISTENTE` | (ago 2026) Vista de administrador SIN autorizar: ve todos los módulos del admin **excepto Usuarios y Ajustes**, con visibilidad global (agenda de todos, contratos, liquidaciones, incrementos, solicitudes, dashboard, rastreo, inmuebles). NO aprueba/devuelve, NO registra pagos, NO aplica/anula incrementos ni envía cartas. SÍ crea, edita, reasigna y **confirma por WhatsApp** visitas en la agenda (#71; confirmar es gestión de agenda: `PATCH /visits/:id/confirm` acepta dueño o staff y el mensaje sale a nombre del agente asignado) — el responsable se elige SIEMPRE explícito (el backend rechaza con 400 una visita del asistente sin `assignedUserId`). **Sep 2026: el asistente también es asignable y ejecuta SUS visitas** — aparece en el selector de agente (Agenda filtra `AGENT` + `ASISTENTE`), sale en `/api/users/locations` (Rastreo y mapa de la agenda) y en las visitas asignadas a él hace check-in/finalizar/no atendida/cancelar/fotos/eliminar como un agente (geofencing de agente, 1500 m). En las AJENAS solo gestiona la agenda: el middleware `forbidAsistente` se eliminó y la restricción vive en cada controlador de ejecución ("dueño o ADMIN" → 403), en `puedeEjecutar(visit)` de Agenda.jsx (oculta acciones y el clic a `/visit/:id`) y en `VisitExecution` (pantalla de error si la visita no es suya). El cron de check-in horario (`locationReminders.js`) sigue pingueando solo a `AGENT`. SÍ gestiona solicitudes (radicar, notas, estados, asignar responsable) y envía comunicados desde Rastreo. SÍ diligencia contratos como un agente (ago 2026): crea, edita, envía a revisión y comparte SUS contratos (`esMio` en Contracts.jsx — dueño o admin); los ajenos siguen de solo lectura y aprobar/devolver sigue siendo del admin. Implementación: helper isomorfo `esStaff(role)` en `src/utils/roles.js` + middleware `requireStaff` en `auth.middleware.js`; "staff" = visibilidad, las autorizaciones siguen chequeando `ADMIN` |
