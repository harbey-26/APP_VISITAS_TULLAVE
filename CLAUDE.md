...
| `ASISTENTE` | (ago 2026) Vista de administrador SIN autorizar: ve todos los módulos del admin **excepto Usuarios y Ajustes**, con visibilidad global (agenda de todos, contratos, liquidaciones, incrementos, solicitudes, dashboard, rastreo, inmuebles). NO aprueba/devuelve, NO registra pagos, NO aplica/anula incrementos ni envía cartas, NO ejecuta ni cierra visitas (check-in/finalizar/no atendida/cancelar/fotos/eliminar bloqueados, `/visit/:id` bloqueado). SÍ crea, edita, reasigna y **confirma por WhatsApp** visitas en la agenda (#71; confirmar es gestión de agenda: `PATCH /visits/:id/confirm` acepta dueño o staff y el mensaje sale a nombre del agente asignado) — siempre asignadas a un agente (el backend rechaza con 400 una visita del asistente sin `assignedUserId`). SÍ gestiona solicitudes (radicar, notas, estados, asignar responsable) y envía comunicados desde Rastreo. SÍ diligencia contratos como un agente (ago 2026): crea, edita, envía a revisión y comparte SUS contratos (`esMio` en Contracts.jsx — dueño o admin); los ajenos siguen de solo lectura y aprobar/devolver sigue siendo del admin. Implementación: helper isomorfo `esStaff(role)` en `src/utils/roles.js` + middlewares `requireStaff` / `forbidAsistente` en `auth.middleware.js`; "staff" = visibilidad, las autorizaciones siguen chequeando `ADMIN` |
