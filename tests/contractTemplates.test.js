...
describe('buildContractDocument', () => {
    it('administración: interpola datos en cuadro resumen y cláusulas', () => {
        ...
        expect(doc.title).toContain('administración');
        ...
        const texto = doc.blocks.filter((b) => b.kind === 'paragraph').map((b) => b.text).join('\n');
        expect(texto).toContain('CLÁUSULA VIGÉSIMA PRIMERA');
        expect(texto).toContain('Ley 527 de 1999');
    });

    it('arrendamiento: suma canon + administración y lista deudores', () => {
        const data = {
        ...
        const texto = doc.blocks.filter((b) => b.kind === 'paragraph').map((b) => b.text).join('\n');
        expect(texto).toContain('UN MILLÓN DOSCIENTOS MIL PESOS ($1.200.000)'); // 1.047.100 + 152.900
    ...
    it('sin cuota de administración usa la variante "no aplica"', () => {
        ...
        const doc = buildContractDocument('ARRENDAMIENTO', data);
        const quinta = doc.blocks.find((b) => b.kind === 'paragraph' && b.text.startsWith('QUINTA'));
